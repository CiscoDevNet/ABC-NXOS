...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='NXOS Logging')
    parser.add_argument('--log', default="DEBUG", help="logging level [FATAL, ERROR, WARN, INFO, DEBUG]")
    parser.add_argument('--logfile', default="nxos.log")
    args = parser.parse_args()
    logger = default("nxos-logging", level=args.log, logfile=args.logfile)
    logger.debug(os.environ)
    logger.info("creating nxos vlan")
    logger.warning("using default password: Cisco12345")

    logger.fatal("program has encountered a critical error")
    logger = default("nxos-basics", patterns=["Cisco12345"], level=args.log, logfile=args.logfile)
